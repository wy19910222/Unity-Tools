...
/// <summary>
/// Example script that uses EasySplinePath2D to generate a mesh that follows 
/// the spline.
/// </summary>
[RequireComponent(typeof(EasySplinePath2D))]
[RequireComponent(typeof(MeshFilter))]
[RequireComponent(typeof(MeshRenderer))]
public class EasyRoad : MonoBehaviour
{
    public float trackWidth = 3;
    [Range(.05f, 1.5f)]
    public float segmentLenght = 1;
    public float tiling = 1;
	public bool liveUpdate;

	public void UpdateMesh()
    {
        SplinePath2D spline = GetComponent<EasySplinePath2D>().path;
        Vector2[] points = spline.GetEquidistancePoints(segmentLenght);
        GetComponent<MeshFilter>().mesh = CreateMesh(points, spline.IsClosed, trackWidth);
        int texture = Mathf.RoundToInt(tiling * points.Length * segmentLenght * .05f);
